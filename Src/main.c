...
extern volatile uint32_t
	timeoutCnt;    // Timeout counter for the General timeout (PPM, PWM, Nunchuck)
extern uint8_t
	timeoutFlagADC;          // Timeout Flag for for ADC Protection: 0 = OK, 1 = Problem detected (line disconnected or wrong ADC data)
extern uint8_t
...
//   int16_t lastCmd1 = 0, lastCmd2 = 0;
  int16_t speedL = 0, speedR = 0;
  int16_t lastSpeedL = 0, lastSpeedR = 0;

  int32_t board_temp_adcFixdt = adc_buffer.temp
	  << 16;  // Fixed-point filter output initialized with current ADC converted to fixed-point
  ...
  while (1) {
	...
	if (main_loop_counter % 25 == 0) {    // Send data periodically every 125 ms
	  setScopeChannel(0, (int16_t)cmd1);                    // 1: INPUT1
	  setScopeChannel(1, (int16_t)cmd2);                    // 2: INPUT2
...
//	  setScopeChannel(7, (int16_t)board_temp_deg_c);          // 8: for verifying board temperature calibration
	  setScopeChannel(4, (int16_t)Sideboard_L.roll);
	  setScopeChannel(5, (int16_t)Sideboard_L.pitch);
	  setScopeChannel(6, (int16_t)Sideboard_L.yaw);
//	  setScopeChannel(7, (uint16_t)Sideboard_L.sensors & (uint8_t)SENSOR1_SET);
//	  setScopeChannel(12, (uint16_t)(Sideboard_L.sensors & (uint8_t)SENSOR2_SET) >> 1u);
//	  setScopeChannel(13, (uint16_t)(Sideboard_L.sensors & (uint8_t)SENSOR_MPU) >> 2u);
	  consoleScope();
	}
	#endif
